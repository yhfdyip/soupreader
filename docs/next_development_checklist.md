...
- ✅ 登录态请求头/登录信息持久化，并在请求链自动并入登录头。

### 1.2 未完成（⚠️）

- ⚠️ legado 规则全集仍未达“声明级 100% 覆盖”（详见 `docs/LEGADO_SOURCE_PARITY_MATRIX.md` 的差距与 Todo）。
...
- [x] 建立《legado 规则对齐矩阵》并逐项打勾（字段/语义/边界）（见 `docs/LEGADO_SOURCE_PARITY_MATRIX.md`）。
...
- [ ] 复核书源界面流程（导入/调试/编辑）与 legado 操作路径一致性。
